...
def build_rules_tab(frame: tk.Misc, is_hexidirect: tk.BooleanVar, on_mode_change) -> Tuple[tk.Label, tk.Text]:
    ...
    tk.Radiobutton(
        mode_frame, text="Conway", variable=is_hexidirect, value=False, command=on_mode_change
    ).pack(side=tk.LEFT, padx=(6, 0))
    tk.Radiobutton(
        mode_frame, text="HexiDirect", variable=is_hexidirect, value=True, command=on_mode_change
    ).pack(side=tk.LEFT, padx=(6, 0))
    ...
    rule_scrollbar = tk.Scrollbar(text_frame, orient=tk.VERTICAL, command=rule_text.yview)
...
def build_run_tab(frame: tk.Misc, on_step) -> tk.Label:
    btns = tk.Frame(frame)
    btns.pack(fill=tk.X, padx=8, pady=6)
    tk.Button(btns, text="Step", command=on_step, bg="lightgreen").pack(side=tk.LEFT, padx=3)

...
class Frame:
    ...
    def set_content(self, lines: List[str], tags: List[List[Tuple[int, int, str]]] = None):
        """Set the content lines and tags for this frame."""
        self.content_lines = lines
        self.content_tags = tags or [[] for _ in lines]
        
    def render_to(self, canvas: List[List[str]], tag_canvas: List[List[List[Tuple[int, int, str]]]]):
        ...
        draws_center = self.x == 0 and self.width == WIDTH // 2  # Left frame draws center border
...
class AsciiUILayout:
    ...
    def render(self) -> Tuple[List[str], List[List[Tuple[int, int, str]]]]:
        ...
        tag_canvas = [[] for _ in range(HEIGHT)]
        ...
        history_content = []
        history_content.append(f"Generation: {self.controller.history_current_index():3} | Active: {active:3}")
        ...
        logs = self.controller.history_get_logs(self.controller.history_current_index()) if history else []
        for log in logs[:10]:

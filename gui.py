...
Unified GUI supporting both Conway-style t        # Text area for rules
        text_frame = tk.Frame(rules_frame)
        text_frame.pack(fill=tk.BOTH, expand=True, pady=2)
        
        self.rule_text = tk.Text(text_frame, height=4, font=("Consolas", 10))
        rule_scrollbar = tk.Scrollbar(text_frame, orient=tk.VERTICAL, command=self.rule_text.yview)
        ...
        # Default rule
        self.rule_text.insert("1.0", "B3/S23")es and HexiDirect symbolic rules.
"""
...
class HexiRulesGUI:
    ...
    def create_controls_panel(self, parent):
        ...
        tk.Radiobutton(mode_frame, text="Conway/Totalistic Rules", 
                      variable=self.is_hexidirect, value=False,
                      command=self.on_mode_change).pack(anchor=tk.W)
        tk.Radiobutton(mode_frame, text="HexiDirect Symbolic Rules", 
                      variable=self.is_hexidirect, value=True,
                      command=self.on_mode_change).pack(anchor=tk.W)
        ...
        self.rule_label = tk.Label(rules_frame, text="Enter Conway rules (e.g., B3/S23):")
        ...
        tk.Button(button_frame, text="Step", command=self.step, bg="lightgreen").pack(side=tk.LEFT, padx=2)
        tk.Button(button_frame, text="Clear", command=self.clear, bg="lightcoral").pack(side=tk.LEFT, padx=2)
        tk.Button(button_frame, text="Random", command=self.randomize, bg="lightyellow").pack(side=tk.LEFT, padx=2)
        
        # Status and help
        status_frame = tk.LabelFrame(controls_frame, text="Status & Help", padx=10, pady=5)
        ...
        self.help_label = tk.Label(status_frame, 
            ...
            font=("Arial", 9), justify=tk.LEFT, wraplength=CONTROLS_WIDTH-40)
        self.help_label.pack(fill=tk.X, pady=2)
        
        # Comprehensive processing log
        log_frame = tk.LabelFrame(controls_frame, text="Processing Log", padx=10, pady=5)
        log_frame.pack(fill=tk.BOTH, expand=True, pady=5)
        
        log_text_frame = tk.Frame(log_frame)
        log_text_frame.pack(fill=tk.BOTH, expand=True)
        
        self.log_text = tk.Text(log_text_frame, height=12, font=("Consolas", 9), wrap=tk.WORD)
        ...
        # Clear log button
        tk.Button(log_frame, text="Clear Log", command=self.clear_log, bg="lightblue").pack(pady=2)
    ...
    def create_grid_panel(self, parent):
        ...
        self.canvas = tk.Canvas(grid_frame, width=GRID_WIDTH-20, height=WINDOW_HEIGHT-20, bg="lightgray")
    ...
    def draw_hex(self, x: float, y: float, color: str, state: str = "", direction: Optional[int] = None) -> int:
        ...
        hex_id = self.canvas.create_polygon(points, fill=color, outline="black", width=1)
        ...
        if self.is_hexidirect.get():
            ...
            if state and state != "_":
                if direction is not None:
                    ...
                    text = state
                self.canvas.create_text(x, y, text=text, font=("Arial", 8, "bold"), fill="black")
            ...
            if direction is not None:
                ...
                dot_y = y - dot_distance * math.sin(angle)  # Negative because screen Y increases downward
                self.canvas.create_oval(dot_x - 3, dot_y - 3, dot_x + 3, dot_y + 3, 
    ...
    def update_display(self):
        ...
        if self.is_hexidirect.get():
            ...
            active_cells = sum(1 for q in range(-GRID_RADIUS, GRID_RADIUS + 1) 
                             for r in range(-GRID_RADIUS, GRID_RADIUS + 1)
        ...
        self.status_label.config(text=f"Active cells: {active_cells} | Rules: {current_rules}")
    ...
    def on_mode_change(self):
        ...
        if self.is_hexidirect.get():
            ...
            self.help_label.config(text="HexiDirect mode: Left click: cycle states | Right click: cycle directions | Middle click: clear")
        else:
    ...
    def on_middle_click(self, event: Any):
    ...
    def log_rule_applications(self):
        ...
        for (q, r), cell in self.hex_automaton.grid.items():
            for rule in self.hex_automaton.rules:
                ...
                if rule.source_state == cell.state:
                    if rule.source_direction is None or rule.source_direction == "random" or rule.source_direction == cell.direction:
                        if self.hex_automaton.matches_condition(cell, q, r, rule):
                            ...
                            if result and result != cell:
                                self.log_message(f"  Rule '{rule.rule_str}' applies to ({q},{r}):{cell} -> {result}")
        
        self.log_message(f"Checked {checked_count} rule-cell combinations, found {match_count} matches")
        self.log_message("Note: When multiple rules from same macro match, one is chosen randomly")
